...
const CNBC_MONTHS: Record<string, number> = {
  january: 1,
  february: 2,
  march: 3,
  april: 4,
...
const CNBC_URL = "https://www.cnbc.com";

export class CnbcScraper implements NewsScraper {
  name = "CNBC";
  baseUrl = CNBC_URL;

  dateParser = createBaseDateParser(CNBC_MONTHS, new Date());
  ...
  async scrapeIndex(startDate: Date, kanal?: string): Promise<NewsItem[]> {
    ...
    const articleElements = doc.querySelectorAll("article[data-testid='article']");

    for (const article of articleElements) {
      const linkElement = article.querySelector("a[href]");
      const titleElement = article.querySelector("h2");
      const dateElement = article.querySelector("time");
      const categoryElement = article.querySelector("span[data-testid='category']");
